...
int_t AR_vscwprintf(const wchar_t *fmt, va_list args)
{
		...
		while(*fmt)
		{
				...
				}else if(*fmt == L'l' && *(fmt + 1) == L'l')/*gcc一族编译器*/
				{
						fmt += 2;
						modifier = __MODIFIER_INT64;
				}else
...
static const wchar_t* __wtou64_s(const wchar_t *in, const wchar_t *end, uint64_t  *num, size_t base, uint32_t flag)
{
		uint64_t val,digit, maxval;
		const wchar_t *p;
...
const wchar_t* AR_wtoi32_s(const wchar_t *in, const wchar_t *end, int32_t  *num, size_t base)
{
		...
		if(p != NULL)
		{
				if(val < AR_INT32_MIN)
				{
						p = NULL;
						errno = ERANGE;
						val = (uint32_t)(-AR_INT32_MIN);
				}else if(val > AR_INT32_MAX)
				{
						p = NULL;
						errno = ERANGE;
						val = (int32_t)AR_INT32_MAX;
				}else
				{
				}
		}else
		{
				if(errno == ERANGE)
				{
						val = val == AR_INT64_MAX ?  (int64_t)AR_INT32_MAX : (uint32_t)(-AR_INT32_MIN);
				}
		}
...
const wchar_t* AR_wtou32_s(const wchar_t *in, const wchar_t *end, uint32_t *num, size_t base)
{
		...
		if(p != NULL)
		{
				if(val > AR_UINT32_MAX)
				{
						p = NULL;
						errno = ERANGE;
						val = (uint32_t)AR_UINT32_MAX;
				}
		}else
		{
				if(errno == ERANGE)val = (uint32_t)AR_UINT32_MAX;
		}
...
*/
		...
		AR_ASSERT(in != NULL && num != NULL);
		return AR_wtod_s(in, in + AR_wcslen(in) + 1, num);
}
