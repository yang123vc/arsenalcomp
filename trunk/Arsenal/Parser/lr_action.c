		...
		for(i = 0; i < tbl->row; ++i)
		{
				...
				for(j = 0; j < tbl->col; ++j)
				{
						...
						case PSR_REDUCE:
						{
								...
								rule = pact->rule;
								AR_swprintf(buf, 1024, L"[<%ls>:%d]",rule->head->name, pact->reduce_count);
								AR_AppendFormatString(str,L"%*ls", __WIDTH__,buf);
...
void PSR_ReportConflict(const psrActionTable_t *tbl, const psrGrammar_t *grammar, arString_t *str)
{
		...
		for(i = 0; i < tbl->row; ++i)
		{
				for(j = 0; j < tbl->col; ++j)
				{
						...
						AR_AppendFormatString(str,L"state[%d] : %ls\r\n",i, tbl->term_set.lst[j]->name);
...
const static psrAction_t error_action = {PSR_ERROR, 0, NULL, 0, 0,0, NULL};
...
void __insert_action_to_action_list(psrAction_t **dest, const psrAction_t *sour, const psrTermInfo_t *lookahead)
{
