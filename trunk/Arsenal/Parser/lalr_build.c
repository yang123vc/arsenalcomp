		...
		for(node = start->all_config->head; node != NULL; node = node->next)
		{
				...
				*/
				...
				for(inner_node = start->all_config->head; inner_node != NULL; inner_node = inner_node->next)
				{
						...
						new_config = Parser_FindFromConfigList(goto_list, inner_node->config->rule, inner_node->config->delim + 1);

						if(new_config == NULL)
						{
								new_config = Parser_InsertToConfigListByValue(goto_list, inner_node->config->rule, inner_node->config->delim + 1);
						}
						...
						for(i = 0; i < grammar->count; ++i)
						{
								...
								if(Parser_CompSymb(symb, inner_rule->head) == 0)
								{
										...
										*/
										...
										if(new_config == NULL)
										{
												new_config = Parser_InsertToConfigListByValue(all_config, inner_rule, 0);
										}
...
static void __build_slr_actions(lalrStateSet_t *set, const psrSymbMap_t *follow_set)
{
		...
		for(i = 0; i < set->count; ++i)
		{
				...
				for(node = state->all_config->head; node != NULL; node = node->next)
				{
						...
						{
								size_t x;
								const psrSymbList_t *lst = &(Parser_GetSymbolFromSymbMap(follow_set, node->config->rule->head)->lst);
								...
								for(x = 0; x < lst->count; ++x)
								{
										...
										if(Parser_CompSymb(node->config->rule->head, PARSER_StartSymb) == 0)
										{
...
lalrState_t*	Parser_Create_LR0_State(const psrGrammar_t *grammar)
{
		...
		first_cfg = Parser_InsertToConfigListByValue(basis, Parser_GetStartRule(grammar), 0);
				...
				for(i = 0; i < set->count; ++i)
				{
						...
						for(node = state->all_config->head; node != NULL; node = node->next)
						{
								...
								for(fp = node->config->forward->head; fp != NULL; fp = fp->next)
								{
										...
										{
												if(Parser_InsertToSymbList_Unique(&next_config->follow_set, node->config->follow_set.lst[x]))
												{
														changed = true;
