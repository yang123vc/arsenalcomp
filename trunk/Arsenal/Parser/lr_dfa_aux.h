...
typedef struct __follow_table_tag
{
		psrFollowRec_t	*tbl;
		size_t			row;
		size_t			col;
...
inline bool_t  __calc_follow(const psrSymbList_t *body, size_t start_idx, const psrSymbMap_t *first_set, psrSymbList_t *out)
{
		size_t i;
		bool_t has_epsilon;
...
inline void PSR_InitFollowTbl(psrFollow_t *tbl, const psrGrammar_t *grammar)
{
		...
		for(i = 0; i < tbl->row; ++i)
		{
				...
				for(j = 0; j <= rule->body.count; ++j)
				{
						...
						first_rec->has_epsilon = __calc_follow(&rule->body, j + 1, &first_set, &first_rec->lst);
				}
		}
