...
typedef struct __parser_config_node_tag
{
		cfgNodeType_t	type;
		union{
				cfgNodeList_t	lst;
...
psrNode_t*		AR_STDCALL __build_token_decl(psrNode_t **nodes, size_t count, const wchar_t *name, void *ctx)
{
		cfgNode_t		**ns = (cfgNode_t**)nodes;
		cfgNode_t		*res;
...
static psrGrammar_t* __build_grammar()
{
		...
		PSR_InsertRule3(gmr, L"root_rule	: 	token_rule  prec_rule rules_rule", NULL, __build_root);
		
		/**************************************************root*****************************************************end*/

		PSR_InsertRule3(gmr, L"token_rule		: TOK_DEF { token_decl_list ; }", NULL, __build_token_rule);
		PSR_InsertRule3(gmr, L"token_decl_list 	: token_decl_list , token_decl", NULL, __build_token_list);
		PSR_InsertRule3(gmr, L"token_decl_list	: token_decl", NULL, __build_token_decl);
		PSR_InsertRule3(gmr, L"token_decl		: LEXEME : NUMBER", NULL, __build_term);


		PSR_InsertRule3(gmr, L"prec_rule	 :	PREC_DEF { prec_decl_list }", NULL, __build_prec_rule);
		PSR_InsertRule3(gmr, L"prec_rule	 :	", NULL, NULL);

		
		PSR_InsertRule3(gmr, L"prec_decl_list :	prec_decl_list  prec_decl", NULL, __build_prec_decl_list1);
		PSR_InsertRule3(gmr, L"prec_decl_list :  prec_decl", NULL, __build_prec_decl_list2);
		
		PSR_InsertRule3(gmr, L"prec_decl	:	ASSOC_DECL prec_token_list ;", NULL, __build_prec_decl);
		
		
		PSR_InsertRule3(gmr, L"prec_token_list :	prec_token_list  ,  LEXEME", NULL, __build_prec_token1);
		
		PSR_InsertRule3(gmr, L"prec_token_list :	LEXEME", NULL, __build_prec_token2);

		/*******************************rules*************************************/


		PSR_InsertRule3(gmr, L"rules_rule :	RULE_DEF  { rule_decl_list  }", NULL, __handle_rules);
		PSR_InsertRule3(gmr, L"rule_decl_list	:	rule_decl_list   rule_decl", NULL, __handle_rule_decl_list1);
		PSR_InsertRule3(gmr, L"rule_decl_list	: rule_decl", NULL, __handle_rule_decl_list2);
		
		PSR_InsertRule3(gmr, L"rule_decl		:	LEXEME -> rule_list ;", NULL, __handle_rule_decl);
		
		PSR_InsertRule3(gmr, L"rule_list		:	rule_list  | rule rule_prec", NULL, __handle_rule_list1);
		PSR_InsertRule3(gmr, L"rule_list		:	rule rule_prec", NULL, __handle_rule_list2);
		
		PSR_InsertRule3(gmr, L"rule		:	rule LEXEME", NULL, __handle_rule_list);

		PSR_InsertRule3(gmr, L"rule		:   LEXEME", NULL, __handle_rule_lexeme);
		PSR_InsertRule3(gmr, L"rule		:   .", NULL, __handle_rule_lexeme);
		

			
		PSR_InsertRule3(gmr, L"rule_prec	:	PREC_DEF LEXEME", NULL, __handle_rule_prec);

		PSR_InsertRule3(gmr, L"rule_prec	: ", NULL, NULL);
...
static bool __config_grammar(psrGrammar_t *gmr, cfgNode_t *root)
{
		...
		for(i = 0; i < root->root.term_list->lst.count && is_ok; ++i)
		{
		...
		for(i = 0; i < root->root.prec_list->lst.count && is_ok; i++)
		{
		...
		for(i = 0; i < root->root.rule_list->lst.count && is_ok; ++i)
		{
				const cfgNode_t	*rule = root->root.rule_list->lst.nodes[i];
				AR_ASSERT(rule && rule->type == CFG_RULE_T);
				is_ok = PSR_InsertRule3(gmr, rule->rule.rule_str, rule->rule.prec_token, __def_reduce_handler);
		}
