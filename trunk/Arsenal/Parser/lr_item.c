...
static bool_t	__remove_lr_record(psrLRItemRec_t	**prec, const psrLRItem_t *item)
{
		int_t cmp;
...
static bool_t __insert_lr_record_unique(psrLRItemRec_t **prec, const psrLRItem_t *item)
{
		int_t cmp;
...
static psrLRItemRec_t* __copy_lr_record(const psrLRItemRec_t *sour)
{
...
static psrLRItemRec_t* __find_lr_record(const psrLRItemRec_t *lst, const psrLRItem_t *key)
{
		int_t cmp;
		AR_ASSERT(key != NULL);
...
bool_t	PSR_InsertToLRItemTbl(psrLRItemTbl_t *tbl, const psrLRItem_t *item)
{
		psrLRItemRec_t **prec;
...
bool_t PSR_RemoveFromLRItemTbl(psrLRItemTbl_t *tbl, const psrLRItem_t *item)
{
		psrLRItemRec_t **prec;
		AR_ASSERT(tbl != NULL && item != NULL);
		prec = &tbl->bucket[item->hash_code % LR_ITEM_BUCKET_SIZE];
...
bool_t	PSR_EqualLRItemTbl(const psrLRItemTbl_t *l, const psrLRItemTbl_t *r)
{
		size_t i;
...
bool_t			   PSR_GetLRItemTblNext(const psrLRItemTbl_t *tbl, psrLRItemTblIter_t *iter)
{
