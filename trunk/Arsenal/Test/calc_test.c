...
static  parser_t* __build_parser()
{
		...
		PSR_InsertRule3(gmr, L"E : E + E", NULL, create_node);
		PSR_InsertRule3(gmr, L"E : E - E", NULL, create_node);
		PSR_InsertRule3(gmr, L"E : E * E", NULL, create_node);
		PSR_InsertRule3(gmr, L"E : E / E", NULL, create_node);
		PSR_InsertRule3(gmr, L"E : E % E", NULL, create_node);

		PSR_InsertRule3(gmr, L"E : number", NULL, create_node1);
		PSR_InsertRule3(gmr, L"E : ( E )", NULL, create_node1);
		PSR_InsertRule3(gmr, L"E : - E", L"UMINUS", create_node2);
...
int post_order(Node_t *node)
{
		...
		AR_printf(L"%s : %d", node->op.str, node->op.type);
		switch(node->op.type)
		{
		case ADD:
				return l + r;
