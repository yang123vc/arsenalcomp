...
static void __insert_start_arc(lexDFA_t *vtx, const lexDFAArc_t *arc)
{
		AR_ASSERT(vtx != NULL && arc != NULL && arc->start == vtx);
...
static void __insert_end_arc(lexDFA_t *vtx, const lexDFAArc_t *arc)
{
...
static void __remove_out_arc(lexDFA_t *vtx, size_t pos)
{
		while(pos < vtx->out_count -1)
		{
				vtx->out[pos] = vtx->out[pos+1];
				pos++;
		}
...
static void __remove_in_arc(lexDFA_t *vtx, size_t pos)
{
		while(pos < vtx->in_count -1)
		{
				vtx->in[pos] = vtx->in[pos+1];
				pos++;
		}
...
void	LEX_DisConnect(lexDFA_t *vtx, lexDFAArc_t *arc)
{
		size_t pos;
		size_t i;
		for(pos = 0; pos < vtx->out_count; ++pos)
		{
				if(vtx->out[pos].match_id == arc->match_id)
				{
						break;
				}
		}
		
		if(pos == vtx->out_count)return;

		for(i = 0; i < arc->to->in_count; ++i)
		{
				if(arc->to->in[i].match_id == arc->match_id)
				{
						__remove_in_arc(arc->to, i);
						break;
				}
		}
		__remove_out_arc(vtx, pos);
}
...
void LEX_BuildStateTable(lexStateTable_t *tbl, lexDFA_t *dfa, const lexCClass_t *cclass)
{
		...
		tbl->is_final = AR_NEWARR0(bool, tbl->count);
