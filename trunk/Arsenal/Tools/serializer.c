...
void		__put_int(arBuffer_t	*buffer, const snInteger_t *integer)
{
		...
		*--p = 'e';
		...
		if(is_neg)*--p = '-';
		*--p = 'i';
		AR_InsertToBuffer(buffer, (const byte_t*)p, AR_strlen(p));
}
...
void __put_list(arBuffer_t	*buffer, const snList_t *lst)
{
		...
		AR_InsertToBuffer(buffer, (const byte_t*)h, 1);
		...
		AR_InsertToBuffer(buffer, (const byte_t*)e, 1);
}
