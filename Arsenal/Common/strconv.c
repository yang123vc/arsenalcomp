...
#if(0)
static size_t __utf8_to_unicode_char(const ar_byte_t *utf8, size_t len, wchar_t *uch)
{
		size_t v,n,e;
		const ar_byte_t *p;
...
static size_t __unicode_to_utf8_char(wchar_t uch, ar_byte_t *utf8)
{
		ar_byte_t buf[10];
		ar_byte_t *e;
		ar_uint_32_t	uc = (ar_uint_32_t)uch;
...
size_t AR_wcs_to_utf8(const wchar_t *unicode, size_t n, char *out, size_t out_len)
{
		...
		if(out != NULL)
		{
				...
				for(i = 0; i < n; ++i)p += __unicode_to_utf8_char(unicode[i], (ar_byte_t*)p);
		}
...
{
		const char *p;
		size_t need; ar_int_t l;
		AR_ASSERT(utf8 != NULL && n > 0);
		p = utf8; need = 0; l = (ar_int_t)n;

		while(l > 0)
		{
				size_t nc = __utf8_to_unicode_char((const ar_byte_t*)p, (size_t)l, NULL);
				if(nc == 0)return 0;
				need++;
				p += nc;
				l -= nc;
		...
		if(out != NULL)
		{
				...
				while(l > 0)
				{
						size_t nc = __utf8_to_unicode_char((const ar_byte_t*)p, (size_t)l, &out[need]);
						need++;
						l -= nc;
						p += nc;
				}
