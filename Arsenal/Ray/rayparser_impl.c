...
}__g_term_pattern[] = 
{
		...
		{L"attribute", ATTRIBUTE, 1, L"\"attribute\"", false},
...
/*direct_declarator	:	direct_declarator ( ) */
/*direct_declarator	:	direct_declarator ( error ) */
/*direct_declarator	:	direct_declarator [ error ] */
...
/*compound_statement	:	{ push_symtbl declaration_list statement_list error pop_symtbl } */
...
{
		{L"program  :  translation_unit FAKE_EOI ", NULL, handle_program, 0},
		{L"program  :  error FAKE_EOI ", NULL, handle_program, 0},
...
psrGrammar_t*	__ray_build_grammar_impl(const psrCtx_t	*psr_ctx, const arIOCtx_t *io)											
{
		...
		{																														
				...
				{																												
						...
						{																																					
								PSR_DestroyGrammar(grammar);
								AR_error(AR_ERR_FATAL, L"Failed to build ray grammar  : Prec : \"%ls\"\r\n",   __g_prec_pattern[i].name);
								grammar = NULL;																																
								AR_ASSERT(false);																															
								return NULL;																																
						}
