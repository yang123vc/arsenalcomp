...
static void __build_goto_table(psrActionTable_t *tbl, const lalrStateSet_t *set)
{
		...
		tbl->goto_tbl = AR_NEWARR(int_i_t, tbl->goto_row * tbl->goto_col);
		AR_memset(tbl->goto_tbl, (int_i_t)-1, tbl->goto_row * tbl->goto_col * sizeof(int_i_t));
